...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        # util.raiseNotDefined()
        if self.getLegalActions(state):
            return self.select_random_action(self.computeValueFromQValues(state), state)
        return None

    def select_random_action(self, q_value, state):
        actions = []

        for action in self.getLegalActions(state):
            if q_value is self.getQValue(state, action):
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        # Q[k+1](s, a) = (1 - alpha) * Q(s, a) + alpha * [r + discount * maxQ(s', a')]
